export function getCurrentIteration(
  ...
  for (let i = 0; i < solution_list.length; i++) {
    if (solution_list[i].iteration == it_number) {
      ...
      for (
        let index = 0;
        index < new_solution.objective_values.length;
        index++
      ) {
        const tradeoff_index: string[] = improveObjectiveIth(
          new_solution,
          index
        );
...
export function getIthObjectiveValues(
  solution_list: Solution[],
  obj_number: number
) {
...
export function improveObjectiveIth(
  solution: Solution,
  objective_to_improve: number
) {
