...
<script lang="ts">
  ...
  async function save_request() {
    try {
      rerender = false;
      let endpoint = API_URL + "/nautnavigroup/save-request";
      ...
      if (response.ok) {
        console.log("Saved NAUTILUS request.");
        let body: saveRequestResponse = await response.json();
        return body.id;
      } else {
        throw new Error("Failed to save NAUTILUS method.");
      }
      //
    } catch (err) {
      toastStore.trigger({
        // prettier-ignore
        message: "Oops! Something went wrong. Saving request failed at the backend.",
  ...
  async function handle_caller(action: string) {
    ...
    socketVal.once("executing-" + action, () => {
      state = State.Calculating;
      socketVal.off("execute-" + action);

      socketVal.once("executed-" + action, (requestIds) => {
  ...
  async function handle_initialize() {
    rerender = false;
    try {
      let endpoint = API_URL + "/nautnavigroup/initialize";
...
<NAUT_NAVI_LAYOUT>
  <div slot="preference">
    ...
    <div>
      ...
      {/if}
      {#if state === State.WaitingForOtherSubmissions}
        <div class="variant-filled-primary btn flex-row">
          <SyncLoader size="20" />
          <p>Waiting for other DMs...</p>
        </div>
      {/if}
