...
<script lang="ts">
  ...
  import Visualizations from "$lib/components/methods/xnimbus/TabbedVisualizations.svelte";
  ...
  import NimbusLayout from "$lib/components/methods/xnimbus/XNIMBUSLayout.svelte";
  ...
  async function handle_initialize() {
    try {
      let endpoint = API_URL + "/xnimbus/initialize";
      ...
      const response = await fetch(endpoint, {
        ...
        body: JSON.stringify({
          ...
          method_id: 2, // Backend technically supports this, but we need to add support for it in the UI.
        }),
      });
      ...
      if (response.ok) {
        ...
        //console.log(problemInfo.current_multipliers);
        selected_solutions = [0];
  ...
  async function handle_iterate() {
    ...
    try {
      let endpoint = API_URL + "/xnimbus/iterate";
  ...
  async function handle_intermediate() {
    ...
    try {
      ...
      let endpoint = API_URL + "/xnimbus/intermediate";
  ...
  async function handle_save_solutions() {
    ...
    try {
      ...
      let endpoint = API_URL + "/xnimbus/save";
  ...
  async function handle_final_choice() {
    ...
    try {
      let endpoint = API_URL + "/xnimbus/choose";
