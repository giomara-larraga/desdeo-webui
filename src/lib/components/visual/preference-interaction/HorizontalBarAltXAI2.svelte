...
<script lang="ts">
  ...
  //let currentTradeoff: string| null = null;
  //let arrowTradeoff: number [][]| null = null;
  ...
  function updateSolutionBar(newValue: number) {
    if (chart) {
      ...
      updateSolutionArrows();
      updateTradeoffs();
      //hideTradeoffs();      // Update the reset arrow position and make it visible
      chart.setOption({
        graphic: [
          {
            id: "arrow",
